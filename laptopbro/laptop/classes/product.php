...
<?php 
	...
	*/
	...
	class product
	{
		...
		public function update_product($data,$files,$id){
			...
			if($product_code == "" || $productName=="" || $productQuantity=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type==""){
				...
				if(!empty($file_name)){
					...
					product_remain=productQuantity-product_soldout,
					brandId = '$brand',
					catId = '$category', 
					type = '$type', 
					price = '$price', 
		...
		public function getNumberOfproductByBrand($brandID)
		{
			$query = "SELECT COUNT(productId) FROM tbl_product where brandId = '$brandID'";
			$result = $this->db->select($query);
		...
		public function getAllproduct($start, $pagerows)
		{
			
			$query = "SELECT * FROM tbl_product order by productId desc LIMIT $start, $pagerows";
			$result = $this->db->select($query);
			return $result;
		}

		public function getAllproductByBrand($brandID,$start, $pagerows)
		{
			
			$query = "SELECT * FROM tbl_product where brandId = '$brandID' order by productId desc LIMIT $start, $pagerows";
			$result = $this->db->select($query);
			return $result;
		}

		public function getNumberOfproductByCategory($catID)
		{
			$query = "SELECT COUNT(productId) FROM tbl_product where catId = '$catID'";
			$result = $this->db->select($query);
			$row = $result->fetch_array(MYSQLI_NUM);
			$record = htmlspecialchars($row[0]);
			return $record;
		...
		public function getAllproductByCategory($catID,$start, $pagerows)
		{
			
			$query = "SELECT * FROM tbl_product where catId = '$catID' order by productId desc LIMIT $start, $pagerows";
			$result = $this->db->select($query);
			return $result;
		}
		

		public function getall_hotproduct()
		{
		...
		public function getNumberOfSearchproduct($tukhoa)
		{
			$tukhoa=$this->fm->validation($tukhoa);
			$query = "SELECT COUNT(productId) FROM tbl_product WHERE productName LIKE '%$tukhoa%' ";
			$result = $this->db->select($query);
			$row = $result->fetch_array(MYSQLI_NUM);
			$record = htmlspecialchars($row[0]);
			return $record;
		...
		public function search_product($tukhoa,$start, $pagerows){
			$tukhoa=$this->fm->validation($tukhoa);
			$query=" SELECT * FROM tbl_product WHERE productName LIKE '%$tukhoa%' LIMIT $start, $pagerows ";
			$result=$this->db->select($query);
			return $result;
		}
	}
