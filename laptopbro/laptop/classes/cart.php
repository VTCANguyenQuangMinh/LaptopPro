...
<?php 
	...
	*/
	class cart
	{
		...
		// kt tiền hiển thị trên giỏ hàng
...
// kiểm tra đơn hàng xuất hiện khi đăng nhập
		...
		{
			$query = "SELECT * FROM tbl_order order by id desc LIMIT 5 ";
			$order_new = $this->db->select($query);
			return $order_new;
		}
		public function cancel_order($id,$time,$price)
		{
			$id = mysqli_real_escape_string($this->db->link, $id);
